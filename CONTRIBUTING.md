...
1. Create a separate branch from `dev`, to work on
    ```bash
    git checkout -b feature/#<issue-number>-myfeature dev
    ```
    The convention is to always have `feature/` or `fix/` in the branch name. The `myfeature` part should describe shortly what the feature is about (separate words with `_` or `-`).
